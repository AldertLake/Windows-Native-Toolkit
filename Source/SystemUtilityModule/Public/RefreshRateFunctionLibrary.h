...
public:
    ...
    static bool TestDisplayMode(const FString& MonitorId, FIntPoint Resolution, int32 RefreshRate);
    ...
    static bool ApplyDisplayMode(const FString& MonitorId, FIntPoint Resolution, int32 RefreshRate, float RevertAfterSeconds);
    ...
    /** Returns supported display modes as parallel arrays where each index is one valid resolution and refresh-rate pair for the monitor. Leave Monitor Id empty to use the game window monitor. */
    UFUNCTION(BlueprintPure, Category = "Windows Native Toolkit|Display Operations", meta = (DisplayName = "Get Supported Display Modes"))
    static void GetSupportedDisplayModes(const FString& MonitorId, TArray<FIntPoint>& Resolutions, TArray<int32>& RefreshRates);
    ...
    static bool SetRefreshRate(int32 NewRefreshRate, const FString& MonitorId);
};
